...
if len(sys.argv) > 2:
    ...
    policy_impact_weights = joblib.load(weights_path)
else:
...
def calculate_dynamic_adjustments(latest_economy, latest_policies, target_ranges, policy_vars, economic_vars, policy_impact_weights):
    adjustments = np.zeros(len(policy_vars)) # Initialise all adjustments to 0
    ...
    for i, policy_var in enumerate(policy_vars):
        for j, economic_var in enumerate(economic_vars):
            target_low, target_high = target_ranges.get(economic_var, (0, float('inf'))) # Get the correspondong target range for the economic variable
            current_value = latest_economy[j]

            # Get SHAP impact weight for these variables
            ...
            if economic_var in ["GDP ($M)", "Inflation Rate (%)", "Economic growth (%)", "Q on Q Economic Growth (%)"] and policy_var in ["Govt Expenditure (% of GDP)", "Money supply ($M)"]:
                if current_value < target_low:
                    ...
                    adjustments[i] -= base_adjustment  # Decrease policy instrument
            elif economic_var in ["Unemployment Rate (%)"] and policy_var in ["Govt Expenditure (% of GDP)", "Money supply ($M)"]:
                if current_value < target_low:
                    adjustments[i] -= base_adjustment  # Decrease policy instrument
                elif current_value > target_high:
                    adjustments[i] += base_adjustment  # Increase policy instrument

            elif economic_var in ["GDP ($M)", "Inflation Rate (%)", "Economic growth (%)", "Q on Q Economic Growth (%)"] and policy_var in ["Tax Revenue (% of GDP)", "Interest Rate (%)"]:
                if current_value < target_low:
                    adjustments[i] -= base_adjustment  # Decrease policy instrument
                elif current_value > target_high:
                    adjustments[i] += base_adjustment  # Increase policy instrument

            elif economic_var in ["Unemployment Rate (%)"] and policy_var in ["Tax Revenue (% of GDP)", "Interest Rate (%)"]:
                if current_value < target_low:
                    adjustments[i] += base_adjustment  # Increase policy instrument
                elif current_value > target_high:
                    adjustments[i] -= base_adjustment  # Decrease policy instrument

            elif economic_var in ["Net Exports (PM)"] and policy_var in ["Rate of crawl (%)"]:
                if current_value < target_low:
                    adjustments[i] -= base_adjustment * 5 # Decrease policy instrument
                elif current_value > target_high:
                    adjustments[i] += base_adjustment * 5 # Increase policy instrument


    # Apply the adjustments to the latest policies values to obtain new policies values
    new_policies = latest_policies + adjustments
    govt_expenditure_change = new_policies[0] - latest_policies[0]
    tax_revenue_change = new_policies[1] - latest_policies[1]
    money_supply_change = new_policies[2] - latest_policies[2]
    ...
    return new_policies, predicted_economy



# Define acceptable target ranges for economic variables
target_ranges = {
    'GDP ($M)': (19000, 24000),
    'Unemployment Rate (%)': (15.0, 20.0),
    'Inflation Rate (%)': (3.0, 6.0),
    'Economic growth (%)': (3.0, 4.0),
    'Q on Q Economic Growth (%)': (1.0, 2.5)
}
...
recommended_policies, predicted_economy = calculate_dynamic_adjustments(
    latest_economy, latest_policies, target_ranges, policy_vars, economic_vars, policy_impact_weights
)

# Print Recommendations
print("\nRecommended Macroeconomic Policy:")
for policy, value in zip(policy_vars, recommended_policies):
    print(f"{policy}: {value:.2f}")
