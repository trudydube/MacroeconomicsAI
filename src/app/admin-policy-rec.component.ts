...
@Component({
    selector: "app-admin-policy-rec",
    templateUrl: "./admin-policy-rec.component.html",
    styleUrls: ["./admin-policy-rec.component.css"],
...
export class AdminPolicyRecComponent {
  policyOutput: any = null;
  ...
  downloadPDF() {
    ...
    const reportElement = document.querySelector('.policy-output') as HTMLElement;
    ...
    reportElement.querySelectorAll('.shap-images').forEach(el => el.remove());
    ...
    this.http.post<any>("http://127.0.0.1:5003/get-policies", formData)
        .subscribe(response => {
          ...
          if (response.success) {
            ...
            const lines = response.policy_output.trim().split("\n");

            const shapHeaders = lines[0].split("\t");
            this.shapValues = lines.slice(1, -27).filter((line: string) => line.trim() !== "").map((line: string) => {
            ...
            this.predictions = {
              gdp: parseFloat(lines[lines.length - 26].split(": ")[1]),
              unemploymentRate: parseFloat(lines[lines.length - 25].split(": ")[1]),
              inflationRate: parseFloat(lines[lines.length - 24].split(": ")[1]),
              economicGrowth: parseFloat(lines[lines.length - 23].split(": ")[1]),
              quarterlyGrowth: parseFloat(lines[lines.length - 22].split(": ")[1]),
              incomeDistribution: parseFloat(lines[lines.length - 21].split(": ")[1]),
              netExports: parseFloat(lines[lines.length - 20].split(": ")[1]),
            }

            this.latestVariables = {
              govtExpenditure: parseFloat(lines[lines.length - 17].split(": ")[1]),
              taxRevenue: parseFloat(lines[lines.length - 16].split(": ")[1]),
              moneySupply: parseFloat(lines[lines.length - 15].split(": ")[1]),
              interestRate: parseFloat(lines[lines.length - 14].split(": ")[1]),
              rateOfCrawl: parseFloat(lines[lines.length - 13].split(": ")[1]),
              gdp: parseFloat(lines[lines.length - 12].split(": ")[1]),
              unemploymentRate: parseFloat(lines[lines.length - 11].split(": ")[1]),
              inflationRate: parseFloat(lines[lines.length - 10].split(": ")[1]),
              economicGrowth: parseFloat(lines[lines.length - 9].split(": ")[1]),
              quarterlyGrowth: parseFloat(lines[lines.length - 8].split(": ")[1]),
              incomeDistribution: parseFloat(lines[lines.length - 7].split(": ")[1]),
              netExports: parseFloat(lines[lines.length - 6].split(": ")[1]),
            };
            
            this.policyOutput = {
              govtExpenditure: parseFloat(lines[lines.length - 5].split(": ")[1]),
              taxRevenue: parseFloat(lines[lines.length - 4].split(": ")[1]),
              moneySupply: parseFloat(lines[lines.length - 3].split(": ")[1]),
              interestRate: parseFloat(lines[lines.length - 2].split(": ")[1]),
              rateOfCrawl: parseFloat(lines[lines.length - 1].split(": ")[1]),
            };
          } else {
            this.shapValues = [];
            this.policyOutput = "Error: " + response.error;
            this.isLoading = false;
          }
        });
